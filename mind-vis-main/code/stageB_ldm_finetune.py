...
from dataset import create_Kamitani_dataset, create_BOLD5000_dataset, create_NOD_dataset
...
def main(config):
    ...
    if config.dataset == 'GOD':
        ...
        fmri_latents_dataset_train, fmri_latents_dataset_test = create_NOD_dataset(config.NOD_path, config.patch_size, 
                fmri_transform=fmri_transform, image_transform=[img_transform_train, img_transform_test], 
                subjects=config.NOD_subs)
        num_voxels = fmri_latents_dataset_train.num_voxels
    else:
        raise NotImplementedError

    print('train_set shape: ', fmri_latents_dataset_train.fmri.shape)
    print('test_set shape: ', fmri_latents_dataset_test.fmri.shape)
...
def get_args_parser():
    ...
    parser.add_argument('--NOD_path', type=str)
