...
def get_NOD_stimuli_list(imgLists, sub):
    ...
    for file in imgLists:
        if file.endswith('.txt') and sub in file:
            sti_name += list(np.loadtxt(file, dtype=str))
    for i in range(len(sti_name)):
        sti_name[i] = os.path.basename(sti_name[i])
    return sti_name

def create_NOD_dataset(path='../data/NOD', patch_size=16, fmri_transform=identity,
            image_transform=identity, subjects = NOD_subs, include_nonavg_test=False):
    
    print("inside create nod")#
    fmri_path = path
    fmri_path = os.path.join(path, 'ciftify')
    imgs_dict = np.load(os.path.join(path, 'img_dict.npy'),allow_pickle=True).item()

    fmri_files = []
    imgs = []
    for sub in os.listdir(fmri_path):
        subdir = fmri_path + '/' + sub + '/results/'
        for run in os.listdir(subdir):
            if run.startswith('ses-imagenet'):
                rundir = subdir + run
                for file in os.listdir(rundir):
                    if file.endswith('beta.dscalar.nii'):
                        fmri_files.append(rundir + '/' + file)
                    if file.endswith('.txt'):
                        imgs.append(rundir + '/' + file)
                break
    fmri_files.sort()
    imgs.sort()
    print(fmri_files)
    ...
    for sub in subjects:
        ...
        # load image
        img_files = get_NOD_stimuli_list(imgs, sub)
        img_data_sub = [imgs_dict[name] for name in img_files]
    ...
    num_voxels = fmri_train_major.shape[-1]
    if isinstance(image_transform, list):
        return (NOD_dataset(fmri_train_major, img_train_major, fmri_transform, image_transform[0], num_voxels), 
                NOD_dataset(fmri_test_major, img_test_major, torch.FloatTensor, image_transform[1], num_voxels))
    else:
        return (NOD_dataset(fmri_train_major, img_train_major, fmri_transform, image_transform, num_voxels), 
                NOD_dataset(fmri_test_major, img_test_major, torch.FloatTensor, image_transform, num_voxels))


class NOD_dataset(Dataset):
    def __init__(self, fmri, image, fmri_transform=identity, image_transform=identity, num_voxels=0):
        self.fmri = fmri
        self.image = image
        self.fmri_transform = fmri_transform
